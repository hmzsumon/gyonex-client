...
import {
  Gift, Ticket, Trophy, Clock, ChevronLeft, ChevronRight,
  CheckCircle, AlertTriangle, Sparkles, Calendar,
} from 'lucide-react';
    ...
    <div className="flex items-center gap-2">
      {units.map(([v, l]) => (
        ...
        >
          ...
          <span className="text-[9px] font-bold tracking-widest mt-1 uppercase" style={{ color: 'var(--text-3)' }}>
            {l}
          </span>
        </div>
      ))}
...
export default function LotteryPage() {
  ...
  const buyMutation = useMutation({
    mutationFn: () => lotteryApi.buyTickets(lottery._id, qty),
    onSuccess: () => {
    ...
    onError: (e: { response?: { data?: { message?: string } } }) =>
      showToast(e.response?.data?.message ?? 'Purchase failed', false),
  });
          ...
          <div className="space-y-4">
            {isLoading ? (
              ...
              >
                ...
                />
                <p className="text-base font-bold mb-2" style={{ color: 'var(--text-1)' }}>
                  No Active Lottery
                </p>
              ...
              <>
                ...
                <motion.div
                  ...
                  style={{
                    background: 'linear-gradient(135deg, color-mix(in srgb, var(--warning) 14%, var(--bg-card)), var(--bg-card))',
                ...
                >
                  ...
                  <div className="relative">
                    ...
                    <p className="text-sm font-semibold mt-1" style={{ color: 'var(--text-2)' }}>
                      {lottery.prizeAsset}
                    </p>
                    {lottery.description && (
                      <p className="text-xs mt-3 max-w-sm mx-auto leading-relaxed" style={{ color: 'var(--text-3)' }}>
                        {lottery.description}
                      </p>
                    )}
                  </div>
                ...
                >
                    ...
                    <div className="flex gap-2 flex-wrap mb-3">
                      {QUICK_QTY.map(n => (
                        <button
                          ...
                          style={{
                            background: qty === n ? 'var(--warning)' : 'var(--bg-elevated)',
                            color:      qty === n ? '#0D0A00' : 'var(--text-2)',
                            border:     `1px solid ${qty === n ? 'var(--warning)' : 'var(--border-1)'}`,
                          }}
                        >
                          {n}×
                        </button>
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="font-num" style={{ color: 'var(--warning)' }}>
                  ...
                  >
                    {buyMutation.isPending ? (
                      ...
                      <>
                        ...
                        Buy {qty} Ticket{qty > 1 ? 's' : ''} · {formatUSD(totalCost)}
                      </>
                    )}
                  </button>

                  <p className="text-center text-[10px]" style={{ color: 'var(--text-3)' }}>
                    By purchasing, you agree to the lottery terms. Winner drawn automatically on the draw date.
                  </p>
                </div>
              </>
            )}
              ...
              myHistory.map((item: {
                ...
                <motion.div
                  ...
                  style={{
                    background: item.isWinner
                      ...
                      : 'var(--bg-card)',
                    border: `1.5px solid ${item.isWinner
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <span className={`badge pill text-[10px] ${
                        item.status === 'drawn'    ? 'badge-muted'    :
                        item.status === 'open'     ? 'badge-brand'    :
                                                     'badge-warning'
                      }`}>
                        {item.status}
                      </span>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-sm font-black font-num" style={{ color: 'var(--text-1)' }}>
                      {item.myTickets}×
                    </p>
                    <p className="text-[10px]" style={{ color: 'var(--text-3)' }}>tickets</p>
                  </div>
                </motion.div>
              ))
            )}
          ...
          <div className="space-y-4">
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between">
                <p className="text-xs" style={{ color: 'var(--text-3)' }}>
                  Page {winnersPage} of {totalPages} · {totalWinners} total winners
                </p>
                <div className="flex items-center gap-2">
                  <button
                    ...
                    {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                      ...
                      return (
                        <button
                          ...
                          style={{
                            background: p === winnersPage ? 'var(--brand)' : 'var(--bg-elevated)',
