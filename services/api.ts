...
export const authApi = {
  ...
  enable2FA: (totpCode: string) =>
    apiClient.post("/auth/2fa/enable", { totpCode }),
  disable2FA: (totpCode: string, password: string) =>
    apiClient.post("/auth/2fa/disable", { totpCode, password }),
  changePassword: (data: object) =>
    apiClient.post("/auth/change-password", data),
  sendEmailOtp: (data: object) =>
    apiClient.post("/auth/send-email-change-otp", data),
  changeEmail: (data: object) => apiClient.post("/auth/change-email", data),
};
...
export const userApi = {
  ...
  getLoginHistory: () => apiClient.get("/users/me/login-history"),
  getSessions: () => apiClient.get("/users/me/sessions"),
  terminateSession: (id: string) =>
    apiClient.delete(`/users/me/sessions/${id}`),
};

// ─── Market API ───────────────────────────────────────────────────────────────
export const marketApi = {
  getMarkets: () => apiClient.get("/markets"),
  getTicker: (markets: string) =>
    apiClient.get(`/markets/ticker?markets=${markets}`),
  getOrderbook: (markets: string) =>
    apiClient.get(`/markets/orderbook?markets=${markets}`),
  getCandles: (market: string, unit = "60", count = 200) =>
    apiClient.get(`/markets/${market}/candles?unit=${unit}&count=${count}`),
...
export const walletApi = {
  ...
  confirmDeposit: (data: object) =>
    apiClient.post("/wallets/deposit/confirm", data),
  getDepositWallets: (coin: string) =>
    apiClient.get(`/deposit-wallets/${coin.toUpperCase()}`),
...
export const tradingApi = {
  ...
  getHistory: (params?: object) =>
    apiClient.get("/trading/history", { params }),
  getPortfolio: () => apiClient.get("/trading/portfolio"),
  calcMargin: (data: object) =>
    apiClient.post("/trading/calculator/margin", data),
};
...
export const loansApi = {
  ...
  repay: (id: string, data: object) =>
    apiClient.post(`/loans/${id}/repay`, data),
};
...
export const ranksApi = {
  ...
  acknowledgeImage: (id: string) =>
    apiClient.post(`/ranks/images/${id}/acknowledge`),
...
export const otpApi = {
  ...
  sendSecure: (purpose: string) =>
    apiClient.post("/otp/send/secure", { purpose }),
  verify: (data: object) => apiClient.post("/otp/verify", data),
  verifySecure: (code: string, purpose: string) =>
    apiClient.post("/otp/verify/secure", { code, purpose }),
};
...
export const supportApi = {
  ...
  reply: (id: string, message: string) =>
    apiClient.post(`/support/${id}/reply`, { message }),
};
...
export const p2pApi = {
  ...
  initiateTrade: (orderId: string, data: object) =>
    apiClient.post(`/p2p/orders/${orderId}/trade`, data),
  confirmPayment: (tradeId: string) =>
    apiClient.post(`/p2p/trades/${tradeId}/confirm-payment`),
  releaseFunds: (tradeId: string) =>
    apiClient.post(`/p2p/trades/${tradeId}/release`),
  dispute: (tradeId: string, reason: string) =>
    apiClient.post(`/p2p/trades/${tradeId}/dispute`, { reason }),
};
...
export const adminApi = {
  ...
  freezeUser: (id: string) =>
    apiClient.patch(`/admin/users/${id}/status`, { status: "suspended" }),
  createUser: (data: object) => apiClient.post("/admin/users/create", data),

  // KYC
  getKYC: (params?: object) => apiClient.get("/kyc/admin/list", { params }),
  approveKYC: (id: string, note?: string) =>
    apiClient.patch(`/kyc/admin/${id}/approve`, { note }),
  rejectKYC: (id: string, reason: string) =>
    apiClient.patch(`/kyc/admin/${id}/reject`, { reason }),

  // Withdrawals
  getWithdrawals: (params?: object) =>
    apiClient.get("/admin/transactions", {
      params: { ...(params as object), type: "withdraw" },
    }),
  approveWithdrawal: (id: string) =>
    apiClient.patch(`/admin/transactions/${id}/approve`),
  rejectWithdrawal: (id: string, reason: string) =>
    apiClient.patch(`/admin/transactions/${id}/reject`, { reason }),

  // Loans
  getAllLoans: (params?: object) =>
    apiClient.get("/loans/admin/all", { params }),
  approveLoan: (id: string, data: object) =>
    apiClient.patch(`/loans/admin/${id}/approve`, data),
  rejectLoan: (id: string, reason: string) =>
    apiClient.patch(`/loans/admin/${id}/reject`, { reason }),

  // Financial
  adjustBalance: (data: object) =>
    apiClient.post("/admin/financial/balance/adjust", data),
  bulkCredit: (data: object) =>
    apiClient.post("/admin/financial/balance/bulk-credit", data),
  releaseROI: (data: object) =>
    apiClient.post("/admin/financial/roi/release", data),

  // Trading controls
  pauseTrading: (reason?: string) =>
    apiClient.post("/admin/trading/pause", { reason }),
  resumeTrading: () => apiClient.post("/admin/trading/resume"),

  // Ranks
  getRankClaims: () => apiClient.get("/ranks/admin/pending-claims"),
  uploadRankImage: (data: FormData) =>
    apiClient.post("/ranks/admin/upload-image", data, {
      headers: { "Content-Type": "multipart/form-data" },
    }),
  rejectRankClaim: (userId: string, reason: string) =>
    apiClient.post("/ranks/admin/reject-claim", { userId, reason }),
  disburseSalary: (month: number, year: number) =>
    apiClient.post("/ranks/admin/disburse-salary", { month, year }),

  // Deposit Wallets
  getDepositWallets: () => apiClient.get("/deposit-wallets/admin/all"),
  addDepositWallet: (data: object | FormData) =>
    apiClient.post("/deposit-wallets/admin", data),
  updateDepositWallet: (id: string, data: object) =>
    apiClient.patch(`/deposit-wallets/admin/${id}`, data),
  toggleDepositWallet: (id: string) =>
    apiClient.patch(`/deposit-wallets/admin/${id}/toggle`),
  deleteDepositWallet: (id: string) =>
    apiClient.delete(`/deposit-wallets/admin/${id}`),

  // CMS Media
  uploadMedia: (data: FormData) =>
    apiClient.post("/cms/media/upload", data, {
      headers: { "Content-Type": "multipart/form-data" },
    }),
  ...
  updateBanner: (id: string, data: object) =>
    apiClient.patch(`/cms/banners/${id}`, data),
  ...
  createBlogPost: (data: FormData) =>
    apiClient.post("/cms/admin/blog", data, {
      headers: { "Content-Type": "multipart/form-data" },
    }),
  updateBlogPost: (id: string, data: object) =>
    apiClient.patch(`/cms/admin/blog/${id}`, data),
  deleteBlogPost: (id: string) => apiClient.delete(`/cms/admin/blog/${id}`),

  // Lottery
  createDraw: (data: object) =>
    apiClient.post("/admin/financial/lottery/draw", data),
  getDraws: () => apiClient.get("/admin/financial/lottery/draws"),
  selectWinner: (drawId: string, data: object) =>
    apiClient.post(`/admin/financial/lottery/${drawId}/select-winner`, data),

  // Broadcast
  broadcastNotification: (data: object) =>
  ...
  // Staking
  pauseStake: (stakeId: string) =>
    apiClient.patch(`/admin/financial/staking/${stakeId}/pause`),
};

// ─── DCA Auto-Invest API ──────────────────────────────────────────────────────
export const dcaApi = {
  getPlans: () => apiClient.get("/trading/dca/plans"),
  createPlan: (data: object) => apiClient.post("/trading/dca/plans", data),
  togglePlan: (id: string) =>
    apiClient.patch(`/trading/dca/plans/${id}/toggle`),
  deletePlan: (id: string) => apiClient.delete(`/trading/dca/plans/${id}`),
};

// ─── News API ─────────────────────────────────────────────────────────────────
export const newsApi = {
  getNews: (params?: object) => apiClient.get("/cms/blog", { params }),
  getArticle: (slug: string) => apiClient.get(`/cms/blog/${slug}`),
};

// ─── Lottery API ─────────────────────────────────────────────────────────────
export const lotteryApi = {
  getActive: () => apiClient.get("/lottery/active"),
  buyTickets: (id: string, qty: number) =>
    apiClient.post(`/lottery/${id}/buy`, { quantity: qty }),
  getMyTickets: () => apiClient.get("/lottery/my-tickets"),
  getWinners: (page = 1) =>
    apiClient.get(`/lottery/winners?page=${page}&limit=10`),
};
